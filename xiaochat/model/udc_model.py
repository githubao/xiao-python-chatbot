...
"""
@description: 调用双端编码模型

@author: BaoQiang
@time: 2017/4/25 21:31
"""
...
def create_model_fn(hparams, model_impl):
    def model_fn(features, targets, mode):
        ...
        if mode == tf.contrib.learn.ModeKeys.TRAIN:
            probs, loss = model_impl(hparams, mode, context, context_len, utterance, utterance_len, targets)
        ...
        if mode == tf.contrib.learn.ModeKeys.EVAL:
            ...
            for i in range(9):
                ...
                all_targets.append(tf.zeros([batch_size, 1], dtype=tf.int64))

            probs, loss = model_impl(hparams, mode, tf.concat(0, all_contexts), tf.concat(0, all_context_lens),
